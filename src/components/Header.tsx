    ...
    return (
        <div className='z-40'>
            <header className='bg-primary flex items-center justify-between px-6 md:px-14  shadow-xl w-full z-40' ref={header}>
                <img className='w-32' src={logo_name} alt="Mdd Digital Logo"/>
                <div className='flex text-white gap-8 items-center'>
                    ...
                    <button id="Menu Hambúrguer" aria-label="Menu Hambúrguer" className={IsMenuOpened ? 'hiddenIcon hiddenIcon-active flex md:hidden' : 'hiddenIcon flex md:hidden'} onClick={() => setIsMenuOpened(!IsMenuOpened)}>
                        <div className='flex md:hidden  '></div>
                        <div className='flex md:hidden'></div>
                        <div className='flex md:hidden'></div>
                    </button>
                    <div>
                        <img src={zap} alt="Whats App Link" className='w-10 object-contain shadow-xl mb-1 hover:scale-125 transition-all cursor-pointer' onClick={() => zapLink.current.click()}/>
